    ...
    async def _run(self):
        ...
        try:
            ...
            try:
                ...
                await self._final_q.send_task_result(
                    self._host.name,
                    self._task._uuid,
                    executor_result,
                    task_fields=self._task.dump_attrs(),
                )
                print("final result sent")
...
async def run_async_worker(final_q, task_vars, host, task, play_context, loader, variable_manager, shared_loader_obj, worker_id):
    print("here we go")
    wp = WorkerProcess(final_q, task_vars, host, task, play_context, loader, variable_manager, shared_loader_obj, worker_id)
    print("doing the worker start")
    await wp.start()
    print("shouldn't see this yet...")