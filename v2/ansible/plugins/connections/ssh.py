...
class Connection(ConnectionBase):
    ...
    def connect(self):
        ...
        vvv("ESTABLISH CONNECTION FOR USER: %s" % self._connection_info.remote_user, host=self._host)
    ...
    def exec_command(self, cmd, tmp_path, executable='/bin/sh', in_data=None, sudoable=False):
        ...
        #vvv("EXEC %s" % ' '.join(ssh_cmd), host=self._host)
    ...
    def put_file(self, in_path, out_path):
        ...
        vvv("PUT %s TO %s" % (in_path, out_path), host=self._host)
    ...
    def fetch_file(self, in_path, out_path):
        ...
        vvv("FETCH %s TO %s" % (in_path, out_path), host=self._host)
        cmd = self._password_cmd()
